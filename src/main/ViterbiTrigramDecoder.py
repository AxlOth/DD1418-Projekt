...
class ViterbiTrigramDecoder(object):
    ...
    def viterbi(self, s):
        ...
        # Initiera allt med -inf
        self.v[0,:,:] = -float("inf")
        self.backptr[0,:,:] = Key.START_END

        # Enda tillåtna trigrammet i början är (START, START, k)
        for k in range(Key.NUMBER_OF_CHARS):
            self.v[0, Key.START_END, k] = self.a[Key.START_END][Key.START_END][k] + self.b[index[0]][k]
        ...
        for i in range(1,len(s)):        #len(s)
            for j in range(Key.NUMBER_OF_CHARS):
                for k in range(Key.NUMBER_OF_CHARS):
                    ...
                    kostnad = self.v[i-1,:,j] + self.a[:,j,k] + self.b[index[i],k]      #olika om man ändrar ordning
                    best_väg = np.argmax(kostnad)
                    self.backptr[i,j,k] = best_väg
                    self.v[i,j,k] = kostnad[best_väg]
        ...
        # 1. Hitta bästa slutpar (pp, p)
        pp, p = np.unravel_index(np.argmax(self.v[T - 1]), self.v[T - 1].shape)

        # 2. Börja med endast sista bokstaven (p)
        result = [p]
        ...
        # 3. Backtracka tills vi har T bokstäver
        while len(result) < T:
            # nästa bokstav är prevprev
            result.append(prevprev)

            # "hoppa bakåt" i trädet
            new_prevprev = self.backptr[t][prevprev][prev]

            # skjut fönstret: (pp, p) ← (new_pp, pp)
            prev, prevprev = prevprev, new_prevprev
        ...
        # 5. Konvertera till text
        result = ''.join(Key.index_to_char(x) for x in result)
